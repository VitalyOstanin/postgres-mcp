...
- Maintain `README.md` in English and `README-ru.md` in Russian so both stay aligned with the Postgres MCP feature set.
...
## Project Structure
- `src/`: TypeScript sources for the MCP server, Postgres client, and tool registrations.
...
- **Never use unlimited `Promise.all()` for external operations** - this can cause:
...
- **ALWAYS** for batch operations that make HTTP requests
- **ALWAYS** for operations that access external APIs
- **ALWAYS** for operations that could create many concurrent connections
- **USUALLY** for operations on lists (though typically <10 items)
...
- Single API call with array parameter
...
- Use conservative defaults (≤100 items per page) and document maximum supported sizes.

## MCP Tool Registration
- **Always use the `registerTool()` method** for registering MCP tools as it is the officially recommended approach according to the MCP SDK documentation.
- The `registerTool()` method provides better structure, future-proofing, and enhanced capabilities compared to older methods.

### Method Signatures
- **Preferred**: `registerTool(name, config, handler)`
  - `name`: Tool name (string)
  - `config`: Configuration object with properties:
    - `description`: Detailed usage description (string)
    - `inputSchema`: Zod schema for input validation (optional)
...
- **Avoid**: The overload style of `.tool()` method as support for it is "frozen as of protocol version 2025-03-26. Future additions to tool definition should NOT be added."

### Tool File Export Pattern
Each tool file should export a registration function that accepts the server and client dependencies:
```typescript
// ✅ Correct pattern
...
export function registerSelectTool(server: McpServer, client: PostgreSQLClient) {
  server.registerTool(
    "select",
    {
      description: "Execute SELECT queries against PostgreSQL. Use for: Querying data from PostgreSQL tables.",
...
```typescript
// ✅ CORRECT: Use registerTool() with configuration object
import { registerSelectTool } from "./tools/select.js";

registerSelectTool(this.server, this.client);
```

### Why registerTool() is Better
- Officially recommended approach according to MCP SDK documentation
- Better structure with explicit configuration object
- Enhanced capabilities including outputSchema validation
- Explicit title property instead of complex precedence rules
- Future-proof as it's the actively maintained API
- Supports additional features like _meta field for additional metadata
...
## Data Creation and Relationship Rules
- For data creation tools, consider supporting relationship descriptors to allow clients to build relationship chains during creation.
- After creating data with relationships, always re-fetch the created data or its relationships to confirm the operation was successful; tool descriptions must remind clients to perform this verification step.
