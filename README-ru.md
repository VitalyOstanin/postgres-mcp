...
- По умолчанию пул содержит одно соединение (`--pool-size 1`). При размере 1 два параллельных `tools/call` сериализуются — второй ждёт освобождения клиента первым. Увеличьте `--pool-size`, если ожидаются параллельные вызовы.
