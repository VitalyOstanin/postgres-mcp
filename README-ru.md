...
- Переменные окружения:
  ...
  - `POSTGRES_MCP_OUTPUT_DIRS` — опциональный `:`-разделённый whitelist каталогов, куда разрешена запись `filePath` в инструменте `execute-sql` (по умолчанию: только OS temp). Используйте, если LLM-клиенту нужно сохранять выгрузки рядом с проектом, например `POSTGRES_MCP_OUTPUT_DIRS=/var/data/exports:/srv/dumps`. **Предупреждение по безопасности:** whitelist — зона ответственности оператора, всё что укажете будет доступно MCP-процессу для записи. Не включайте системные и общие каталоги: `/`, `/etc`, `/usr`, `/var`, `/var/log`, `/root`, `/home`, ваш `$HOME` или корень с исходниками; ограничьтесь выделенными каталогами для экспорта.
...
- `coverage/` — генерируется `npm run test:coverage`; HTML-отчёт по покрытию (под gitignore).
- `temp/` — локальный scratch-каталог, используемый тестами (под gitignore).
- `docs/` — локальная документация проекта; в `docs/reviews/` лежат отчёты, формируемые skill-ами `project-check` (весь `docs/` под gitignore, ничего отсюда не публикуется).
...
**Граница привилегий (рекомендация для production):** аутентификация и авторизация полностью делегированы PostgreSQL-роли из `POSTGRES_MCP_CONNECTION_STRING` — у самого MCP-сервера нет понятия пользователя. В production подключайтесь ролью с минимальными правами под вашу нагрузку: не используйте суперпользователей, не выдавайте `pg_read_server_files`, проверяйте все `SECURITY DEFINER`-функции в search_path перед тем, как открывать их через `execute-sql`. По возможности запускайте с дефолтным `--read-only` и без `--auto-connect`, а connection string берите из secret manager, а не из коммитов в репозиторий.
...
- Совет про задержки: если ваш MCP-хост шлёт несколько tool-вызовов параллельно (например, `list-schemas` + `service-info` + `execute-sql` одновременно) и вы хотите, чтобы они шли в БД параллельно, поднимите `--pool-size` до 2–4. Учтите оговорку про multi-call-транзакции выше — всё, что разбивает `BEGIN`/`COMMIT` на несколько вызовов, должно остаться на пуле размера 1.
